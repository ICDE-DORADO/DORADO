...
def resolve_data_base_dir(dataset):
    """Return the directory that contains the dataset folder."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    workspace_dir = os.path.dirname(script_dir) if os.path.basename(script_dir) == "code" else script_dir

    candidate_bases = [
        os.path.join(workspace_dir, "adhub", "ICEWS14", "0.1", "data"),
        os.path.join(workspace_dir, "adhub", dataset, "0.1", "data"),
        os.path.join(script_dir, "..", "adhub", "ICEWS14", "0.1", "data"),
        os.path.join(script_dir, "..", "adhub", dataset, "0.1", "data"),
        os.path.join(os.getcwd(), "adhub", "ICEWS14", "0.1", "data"),
        os.path.join(os.getcwd(), "adhub", dataset, "0.1", "data"),
        os.path.join(os.getcwd(), "..", "adhub", "ICEWS14", "0.1", "data"),
        os.path.join(os.getcwd(), "..", "adhub", dataset, "0.1", "data"),
    ...
    raise FileNotFoundError(
        f"Could not find dataset files for {dataset}. "
        f"Expected server path like ../adhub/ICEWS14/0.1/data/{dataset}/entity2id.txt "
        f"or local path data/{dataset}/entity2id.txt."
    )
...
def load_data(dataset, bfs_level=3, relabel=False):
    ...
    elif dataset in ['ICEWS18', 'ICEWS14', "GDELT", "SMALL", "ICEWS14s", "GDELT","YAGO",
                     "WIKI"]:
        return knwlgrh.load_from_local(resolve_data_base_dir(dataset), dataset)
    else:
...
def get_rank_filter(score, triples):
    ...
    all_triplets_set = set((s, r, o) for s, r, o, t in triples)                      
    ranks = []
    for i in range(T):
        s, r, o, t = triples[i]
...
def get_metric_known_both(rank, valid_triples, emerging_index):
    """
    Calculate metrics for known entities (non-emerging entities).
    Known entities are all entities that are NOT in the emerging_index.
    """
    rank = torch.cat(rank)
    valid_triples = np.concatenate(valid_triples, axis=0)
    total_indices = torch.arange(len(rank), device=rank.device, dtype=torch.long)
    
    # Get known entity indices (all indices except emerging)
    emerging_set = set(emerging_index.cpu().numpy() if isinstance(emerging_index, torch.Tensor) else emerging_index)
    known_indices = torch.tensor([i for i in range(len(rank)) if i not in emerging_set], 
                                  device=rank.device, dtype=torch.long)
    
    if len(known_indices) == 0:
        # If no known entities, return zeros
        return 0.0, 0.0, 0.0, 0.0
    
    rank_known = rank[known_indices]
    mrr = torch.mean(1.0 / rank_known.float())
    hits1 = torch.mean((rank_known <= 1).float())
    hits3 = torch.mean((rank_known <= 3).float())
    hits10 = torch.mean((rank_known <= 10).float())
    return mrr.item(), hits1.item(), hits3.item(), hits10.item()
